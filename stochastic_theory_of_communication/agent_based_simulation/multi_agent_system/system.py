...
class MultiCell():
    ...
    def step_of_cell(self, cell_current_state, not_permitted_neighbors):
        ...
        if tuple(cell_next_state) not in not_permitted_neighbors:
            assert tuple(cell_next_state) in [tuple(posible_state) for posible_state in Cell.STATE_SPACE] # TODO remove assert
    ...
    def set_temperature(self, temperature):
        self.__temperature = min(max(temperature, 0), Cell.STATE_SPACE_SIZE)    
